...
class SuperAdminCondominiumService:
    ...
    async def get_condominium(self, condominium_id: str) -> Optional[CondominiumDetail]:
        ...
        return CondominiumDetail(
            id=str(condo.id),
            name=condo.name,
            slug=getattr(condo, 'slug', None),
            address=getattr(condo, 'address', None),
            phone=getattr(condo, 'phone', None),
            email=getattr(condo, 'email', None),
            plan_type=condo.plan_type,
            is_active=condo.is_active,
            is_setup_completed=condo.is_setup_completed,
            logo_url=getattr(condo, 'logo_url', None),
            default_monthly_fee=getattr(condo, 'default_monthly_fee', None),
            total_users=total_users,
            total_units=total_units,
